...
class Adafruit_I2C:
    ...
    def __init__(self,
                 address,
                 bus=smbus.SMBus(1 if getPiRevision() > 1 else 0),
                 debug=False):
    ...
    def readList(self, reg, length):
        ...
        try:
            ...
            if (self.debug):
                print ("I2C: Device 0x%02X returned the following from reg " +
                       "0x%02X" % (self.address, reg))
                print results
            return results
        except IOError, err:
            print ("Error accessing 0x%02X: " +
        ...
        try:
            ...
            if (self.debug):
                print ("I2C: Device 0x%02X returned 0x%04X from reg " +
                       "0x%02X" % (self.address, result & 0xFFFF, reg))
            return result
        except IOError, err:
            print ("Error accessing 0x%02X: " +
                   "Check your I2C address" % self.address)
